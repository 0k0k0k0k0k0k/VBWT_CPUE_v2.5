# build_site_caches_exclude_self.R
...
cache_dir <- file.path(
  base_dir,
  "data_processed/caches_exclude_self"
)

summary_file <- file.path(
  base_dir,
  "data_processed/vbwt_site_cache_index_exclude_self.csv"
)

filtered_ebd_file <- file.path(
  base_dir,
  "data_processed/ebd_vbwt_filtered_exclude_self.txt"
)
...
hotspot_md5_file <- file.path(
  base_dir,
  "data_processed/vbwt_hotspot_ids_md5_exclude_self.txt"
)
...
if ("OBSERVER_ID" %in% names(ebd)) {
  ebd[, OBSERVER_ID := tolower(trimws(as.character(OBSERVER_ID)))]
}
...
# ---------------------------
# REMOVE MY CHECKLISTS
# ---------------------------
my_ids <- c("obsr497680", "obsr5260421")

if ("OBSERVER_ID" %in% names(ebd)) {
  message(
    "Rows matching my IDs before removal: ",
    format(ebd[OBSERVER_ID %chin% my_ids, .N], big.mark = ",")
  )
  
  # Build a checklist-level ID first so whole checklists can be removed
  if ("GROUP_IDENTIFIER" %in% names(ebd)) {
    ebd[, FILTER_CHECKLIST_ID := fifelse(
      !is.na(GROUP_IDENTIFIER) & GROUP_IDENTIFIER != "",
      GROUP_IDENTIFIER,
      SAMPLING_EVENT_IDENTIFIER
    )]
  } else {
    ebd[, FILTER_CHECKLIST_ID := SAMPLING_EVENT_IDENTIFIER]
  }
  
  # Find every checklist/group tied to either of my observer IDs
  my_checklist_ids <- unique(
    ebd[OBSERVER_ID %chin% my_ids, FILTER_CHECKLIST_ID]
  )
  
  message(
    "Unique checklist/group IDs tied to my observer IDs: ",
    format(length(my_checklist_ids), big.mark = ",")
  )
  
  # Remove the full checklist/group, not just my rows
  ebd <- ebd[!FILTER_CHECKLIST_ID %chin% my_checklist_ids]
  
  message("Rows after checklist-level filter: ", format(nrow(ebd), big.mark = ","))
  
  # Clean up helper column
  ebd[, FILTER_CHECKLIST_ID := NULL]
} else {
  warning("OBSERVER_ID column not found after standardizing names. No observer filtering applied.")
}
